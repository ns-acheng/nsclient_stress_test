...
class NsClientLogValidator:
    def __init__(self, env: IEnvironment):
        self.stagent_path = env.agent_data_dir
        self.log_path = env.agent_log_file
...
def init_validator(env: IEnvironment):
    global _validator
    with _validator_lock:
        if _validator is None:
            _validator = NsClientLogValidator(env)

def get_validator() -> object:
    global _validator
    if _validator is None:
        raise RuntimeError("Validator not initialized. Call init_validator first.")
    return _validator
